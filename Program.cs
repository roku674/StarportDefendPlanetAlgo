...
namespace StarportDefendPlanetAlgo
{
    internal class Program
    {
        private static void Main(string[] args)
        {
            ...
            //Randomization(new int[25, 25], planetMapPath);
            string file = null;
            try
            {
                file = File.ReadAllText(planetMapPath);
            }
            catch (Exception e)
            {
            ...
            if (file != null)
            {
                int[,] mapGrid = JsonConvert.DeserializeObject<int[,]>(file);
        ...
        private static void Randomization(int[,] mapGrid, string planetMapPath)
        {
            mapGrid[12, 12] = 5;
            for (int i = 0; i < 6; i++)
            {
